...
// Distribution Paths (automatically sync to apps for zero-config imports)
const DIST_CONFIG_PATHS = [
  path.join(__dirname, '../frontend/src/shared-generated'),
  path.join(__dirname, '../backend/src/shared-generated')
];

const DIST_ASSETS_PATHS = [
...
`;
  ...
  const presetContent = `/* eslint-disable */
/**
...
async function main() {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  //  DISTRIBUTE ASSETS & CONFIG
  // ═══════════════════════════════════════════════════════════════════════════
  console.log('\n📦 Distributing to apps...');

  // 1. Distribute Config Files (.ts, .css, .js)
  const configFiles = ['branding.ts', 'colors.ts', 'theme.css', 'tailwind.preset.js'];
  DIST_CONFIG_PATHS.forEach(distPath => {
    ...
    console.log(`   ✅ Config synced to ${path.relative(path.join(__dirname, '..'), distPath)}`);
  });

  // 2. Distribute Public Assets (Essential web set only)
  ...
  DIST_ASSETS_PATHS.forEach(distPath => {
    if (fs.existsSync(distPath)) {
      // Optional: Clean up old generated files that might be lingering
      // (Only touches files we are known to generate)
      const filesToPurge = [
        'favicon.svg', 'icon.svg', 'logo-icon.svg',
        'logo-full-light.svg', 'logo-full-dark.svg'
      ];
      // Also purge any icon-*.svg or icon-*.png that isn't in essentials
      const existingFiles = fs.readdirSync(distPath);
      existingFiles.forEach(file => {
        const isLegacySvg = file.endsWith('.svg') && !essentialAssets.includes(file);
        const isLegacyIcon = file.startsWith('icon-') && !essentialAssets.includes(file);
        if (isLegacySvg || isLegacyIcon || filesToPurge.includes(file)) {
          // Safety check: don't delete files we don't manage
          if (file.startsWith('icon-') || file.startsWith('logo-') || ['favicon.svg', 'icon.svg', 'og-image.svg'].includes(file)) {
      ...
      console.log(`   ✅ Essential assets synced to ${path.relative(path.join(__dirname, '..'), distPath)}`);
    }
  });
