...
class VideoProcessorFFmpeg:
    ...
    def apply_vfx(input_path: Path, output_path: Path, ass_path: Path, template: dict, base_dir: Path):
        ...
        # Alpha preserved: inputs are RGBA, PNG/WebM have alpha, overlay inherits automatically
        filter_complex = (
            ...
            # Intro / Outro (opaque videos)
            f"[v_tint][1:v]overlay=0:0:enable='between(t,0,{intro_end})'[v_intro];"
            f"[v_intro][2:v]overlay=0:0:enable='between(t,{outro_start},{duration})'[v_outro];"
            ...
            f"[5:v]scale={target_logo_w}:-1,format=rgba,"
            f"fade=in:st={intro_end}:d={fade_dur}:alpha=1,"
            f"fade=out:st={outro_start - fade_dur}:d={fade_dur}:alpha=1[logo_s];"
            
            f"[v_grad_t][logo_s]overlay=W-w-60:60[v_final]"
        )
